...
if(isset($_POST['Credenciamento'])){
        $credenciamento = $_POST['Credenciamento'];
        $post = array(
                'post_type' =>'credenciamento',
                'post_title'=>$credenciamento['nome'],
                'post_category' => array($cat_id),
                'post_status' => 'publish',
                
        );
        $post_id = wp_insert_post($post);
        wp_set_object_terms($post_id, $credenciamento['funcao'], 'funcao-credenciamento' );
        update_post_meta($post_id, 'email', $credenciamento['email']);
        update_post_meta($post_id, 'midia', $credenciamento['midia']);
        update_post_meta($post_id, 'estado', $credenciamento['estado']);
        update_post_meta($post_id, 'cidade', $credenciamento['cidade']);        
        update_post_meta($post_id, 'acampamento', $credenciamento['acampamento']);
...
<?php $category = get_terms('funcao-credenciamento', 'hide_empty=0');?>
	<div id="container-content" role="main">
		<div id="credenciamento">
			<h1>Credenciamento</h1>
                        ...
                        <?php endif ?>
			<div id="form">
				<form  id="form-credenciamento" action="" method="post">
                                        <div class="RegisterErrors"></div>
					<div class="nome">
						<h2>Nome</h2>
						<input type="text" name="Credenciamento[nome]" class="required">
					</div>
					<div class="e-mail">
						<h2>e-mail</h2>
						<input type="text" name="Credenciamento[email]" class="required email">
					</div>
					<div class="midia">
						<h2>Midia de Comunica&ccedil;&atilde;o</h2>
						<input type="text" name="Credenciamento[midia]">
					</div>
					<div class="estado">
						<h2>Estado</h2>
						<select name="Credenciamento[estado]">
					...
					<div class="cidade" class="required">
						<h2>Cidade</h2>
						<input type="text" name="Credenciamento[cidade]">
					</div>
					<div class="funcao">
						<h2>Fun&ccedil;&atilde;o</h2>
						<select name="Credenciamento[funcao]">
					...
					<div class="acampamento">
						<h2>Acampamento</h2>
                                                <?php query_posts(array ('post_type' => 'acampamento')); ?>
						<select name="Credenciamento[acampamento]">
					...
					<div class="validacao">
						<input type="checkbox" name="Credenciamento[validacao]" value="ok" class="required">
